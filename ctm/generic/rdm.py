...
def rdm2x2(coord, state, env, open_sites=[0,1,2,3],\
    ...
    if False:
        ...
        return rdm2x2_legacy(coord, state, env, sym_pos_def=sym_pos_def, verbosity=verbosity)
...
def rdm2x2_legacy(coord, state, env, sym_pos_def=False, verbosity=0):
    ...
    T2 = env.T[(state.vertexToSite(coord), (-1, 0))]
    dimsA = state.site(coord).size()
    # This line here computes the reduced density matrix of a single site (sti stronzi)
    ...
    #print("-"*50+" SHAPES "+"-"*50)
    #print(f"C2x2_LD, {C2x2_LD.shape}")
    #print(f"C2x2_RD, {C2x2_RD.shape}")
    #print(f"C2x2_LU, {C2x2_LU.shape}")
    #print(f"C2x2_RU, {C2x2_RU.shape}")
    #print(f"C2x2_L, {lower_half.shape}")
    #print(f"C2x2_U, {upper_half.shape}")
    #print("")
...
def rdm2x2_uncontracted(coord, state, env, sym_pos_def=False, verbosity=0):
    r"""
    :param coord: vertex (x,y) specifies upper left site of 2x2 subsystem
    :param state: underlying wavefunction
    :param env: environment corresponding to ``state``
    ...
    T2 = env.T[(state.vertexToSite(coord), (-1, 0))]
    dimsA = state.site(coord).size()
    # These fucking lines here were creating a local rdd before even the contractions. WTF
    a = contiguous(einsum('mefgh,nabcd->eafbgchdmn', state.site(coord), conj(state.site(coord))))
    a = view(a, (dimsA[1] ** 2, dimsA[2] ** 2, dimsA[3] ** 2, dimsA[4] ** 2, dimsA[0], dimsA[0]))
    #a = state.site(coord)
    ...
    return [C2x2_LD, C2x2_RD, C2x2_LU, C2x2_RU]
